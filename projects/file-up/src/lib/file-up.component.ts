...
@Component({
  ...
  template: `
   ...
   <input type="search" [(ngModel)]="filtertext" placeholder="search for file"/>
    <lib-load-table [fileDetails]="opFile" [fileData]="totaldata" [listData]="filteredStudents"></lib-load-table>
...
export class FileUpComponent {
  ...
  pushdata(){
    ...
    setTimeout(() => {
      this.alldata.push({"file":this.opFile,"content":this.totaldata})
